...
@injectable()
export class ChangeUserBlockStatus extends Operation {
    constructor(
    ...
    async execute(userId: number, isBlocked: boolean) {
        const { SUCCESS, BADREQUEST, ERROR } = this.outputs;
        try {
           
            const result = await this.userRepository.changeUserBlockStatus(userId, isBlocked);
            if (result) {
